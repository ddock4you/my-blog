...
`K in keyof T as \`form*${string & K}\`는 각 키를 `form*`접두사가 붙은 새로운 이름으로 변환합니다.`string & K`는 K가 string 타입임을 보장합니다. 이를 통해 User 인터페이스의 모든 필드가 `form_id`, `form_name` 등의 폼 필드 이름으로 변환됩니다.
