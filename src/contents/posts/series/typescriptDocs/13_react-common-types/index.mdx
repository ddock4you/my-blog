...
- **ComponentProps**: 특정 컴포넌트나 HTML 요소의 props 타입을 추출하는 유틸리티 타입입니다. `ComponentProps<'input'>`처럼 제네릭 파라미터와 함께 직접 지정해야 합니다.
...
**ComponentProps**는 특정 컴포넌트나 HTML 요소의 props 타입을 추출하는 유틸리티 타입입니다. `ComponentProps<'input'>`은 HTMLInputElement의 모든 속성을 포함하는 타입이 됩니다. 이를 활용하면 HTML 요소의 속성을 완전히 상속하면서 추가적인 props를 정의할 수 있어, 기존 HTML 요소를 확장한 컴포넌트를 쉽게 만들 수 있습니다.
...
예를 들어 `ComponentProps<'input'>`을 사용하면 `type`, `value`, `onChange`, `placeholder` 같은 HTML input의 모든 표준 속성을 얻을 수 있습니다. 하지만 React의 `ref`나 `key` 같은 속성들은 포함되지 않습니다.
...
**Q. ComponentProps<'input'>은 어떤 타입인가요?**

ComponentProps는 특정 컴포넌트나 HTML 요소의 props 타입을 추출하는 유틸리티 타입입니다. ComponentProps<'input'>은 HTMLInputElement의 모든 속성을 포함하는 타입이 됩니다. 이를 활용하면 HTML 요소의 속성을 완전히 상속하면서 추가적인 props를 정의할 수 있습니다.

**Q. ChangeEvent<HTMLInputElement>는 어떤 타입인가요?**
